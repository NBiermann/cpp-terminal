**Warning**: At the moment, cpp-terminal-utf is under heavy development and <u>should not be used</u> before it reaches version 1.0.0-beta.
...
class Window {
   public :
    ...
    // and only if is_width_fixed() and is_wordwrap() both yield true, but 
...
A child window is always associated with exactly one parent window. 
